...
def save_encodings(config_path, sample_points_file, output_path, by_level=False, encoding_id=None):
...
# load checkpoint
    ...
    pipeline.load_pipeline(loaded_state["pipeline"],pop_appearance_embedding=False, pop_geometry_encoding=False)
    ...
    with torch.no_grad():
        ...
        # if multiscene, use the correct encoder
        if encoding_id:
        ...
        if by_level: # save by resolution level in hashtable
...
def __main__():
    ...
    save_encodings(config_path, sample_points_file, output_path, by_level=args.by_level, encoding_id=args.encoding_id)
